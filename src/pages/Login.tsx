      ...
      <div className="w-full max-w-md">
        <div className="flex items-start gap-5 mb-10">
          ...
          <div className="border-l border-border-light pl-5 py-1">
            <h1 className="brand-title text-3xl text-white leading-none tracking-widest">HANSSENS</h1>
            <p className="label-caps mt-1 mb-3">GROUP</p>
            <div className="h-px w-full bg-border-light mb-3" />
            <p className="label-caps text-faint tracking-[0.25em]">{t('common.tagline')}</p>
          </div>
        </div>

        <div className="border border-border bg-card p-8">
          <div className="mb-6">
            <p className="label-caps text-muted mb-1">{t('login.access')}</p>
            <h2 className="brand-title text-2xl text-white">{t('login.journal')}</h2>
          </div>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="label-caps block mb-2">{t('login.email')}</label>
            ...
            <div>
              <label className="label-caps block mb-2">{t('login.password')}</label>
              <input type="password" value={password} onChange={e => setPassword(e.target.value)} required
                placeholder="••••••••"
                className="w-full bg-bg border border-border px-4 py-3 text-sm text-white placeholder:text-faint focus:outline-none focus:border-border-light transition-colors" />
            </div>
            {error && <p className="text-loss text-xs tracking-wide">{error}</p>}
        ...
        <div className="flex justify-between items-center mt-6">
          <p className="label-caps text-faint">{t('login.belgium')}</p>
